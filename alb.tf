resource "aws_lb" "alb" {
  ...
  #tfsec:ignore:AWS083 - Defaulted to true, but needs to be able to be overridden.
  drop_invalid_header_fields = local.alb.drop_invalid_header_fields
  idle_timeout               = local.alb.idle_timeout
  enable_http2               = local.alb.enable_http2
  ...
  tags = merge({ "Name" = local.alb_name }, local.tags)
}
